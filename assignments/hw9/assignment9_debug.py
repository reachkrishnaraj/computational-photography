...
# cv2.imwrite(output_dir + "/example_hdr.jpg", hdr)
...
# cv2.imwrite(output_dir + "/studio_hdr.jpg", hdr)





def resizeImages(image_dir, factor):
    """ This function reads in input images from a image directory and downsamples them

    Args:
        image_dir(str): The image directory to get images from.
        factor(int): Downsample the images by this factor.
    ...
    for image_idx in xrange(num_images):
        ...
        images_gray[image_idx] = cv2.cvtColor(images[image_idx],
                                              cv2.COLOR_BGR2GRAY)
        images[image_idx] = images[image_idx][::factor,::factor]
        images_gray[image_idx] = images_gray[image_idx][::factor,::factor]

        cv2.imwrite(os.path.join(image_dir, image_files[image_idx]), images[image_idx])

image_dir = "input"
output_dir = "output"
exposure_times = np.float64([1/125.0, 1/50.0, 1/15.0, 1/5.0, 1/2.0])
log_exposure_times = np.log(exposure_times)

np.random.seed()
#resizeImages(image_dir, 2)
hdr = computeHDR(image_dir, log_exposure_times, resize = True)
cv2.imwrite(output_dir + "/skyline_hdr.jpg", hdr)
